...
private:
    ...
    void calCombinationSum(std::vector<int> &nums, std::vector<int> &sums, int index, int sum)
    {
        if (index == (nums.size() + 1) / 2) {sums.push_back(sum); return;}

        calCombinationSum(nums, sums, index + 1, sum);
        calCombinationSum(nums, sums, index + 1, sum + nums[index]);
    }

    void getMinAbsDifference(std::vector<int> &nums, std::vector<int> &sums, int index, int sum, int goal)
    {
        if (res == 0) return;
        ...
        if (index == nums.size())
        {
            ...
            if (l > 0) res = std::min(res, std::abs(sums[l - 1] + sum - goal));
            if (r < n) res = std::min(res, std::abs(sums[r + 1] + sum - goal));
