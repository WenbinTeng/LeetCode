...
private:
    ...
    void dfs(std::vector<int>& aliceArrows, std::vector<int>& path, int restArrows, int index, int score)
    {
        ...
        if (restArrows > aliceArrows[index])
        {
            ...
            dfs(aliceArrows, path, restArrows - aliceArrows[index] - 1, index + 1, score + index);
            path.pop_back();
        }
        path.push_back(0);
        dfs(aliceArrows, path, restArrows, index + 1, score);
