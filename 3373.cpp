...
class Solution {
  public:
    std::vector<int> maxTargetNodes(std::vector<std::vector<int>> &edges1,
        ...
        auto dfs = [](auto &&self, int parent, int node, int color,
                      std::unordered_map<int, std::vector<int>> &edgeDict,
                      std::vector<int> &colorVec,
                      std::vector<int> &nodeCnt) -> void {
            colorVec[node] = color;
            nodeCnt[color]++;
            for (auto next : edgeDict[node]) {
                if (next != parent) {
                    self(self, node, next, color ^ 1, edgeDict, colorVec,
                         nodeCnt);
                }
            }
        };
        ...
        for (int i = 0; i < n; i++) {
            res[i] = nodeCnt1[colorVec1[i]] +
                     (m > 1 ? std::max(nodeCnt2[0], nodeCnt2[1]) : 0);
        }
