  ...
  public:
    TreeNode *buildTree(std::vector<int> &inorder,
        ...
        return inorder.empty() || postorder.empty()
                   ? nullptr
                   : buildNode(inorder, postorder, 0, inorder.size() - 1);
    }

  private:
    int index = 0;

    TreeNode *buildNode(std::vector<int> &inorder, std::vector<int> &postorder,
        ...
        for (int i = inBegin; i <= inEnd; ++i) {
            if (postorder[postorder.size() - 1 - index] == inorder[i]) {
                TreeNode *node =
                    new TreeNode(postorder[postorder.size() - 1 - index++]);
