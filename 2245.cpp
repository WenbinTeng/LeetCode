...
class Solution {
  public:
    int maxTrailingZeros(std::vector<std::vector<int>> &grid) {
        int res = 0;
        std::vector<std::vector<int>> rowCnt2(grid.size(),
                                              std::vector<int>(grid[0].size()));
        std::vector<std::vector<int>> rowCnt5(grid.size(),
                                              std::vector<int>(grid[0].size()));
        std::vector<std::vector<int>> colCnt2(grid.size(),
                                              std::vector<int>(grid[0].size()));
        std::vector<std::vector<int>> colCnt5(grid.size(),
                                              std::vector<int>(grid[0].size()));
        ...
        for (int i = 0; i < grid.size(); ++i) {
            for (int j = 0; j < grid[0].size(); ++j) {
                ...
                res = std::max(res, std::min(prevRowCnt2 + prevColCnt2 + cnt2,
                                             prevRowCnt5 + prevColCnt5 + cnt5));
                res = std::max(res, std::min(prevRowCnt2 + nextColCnt2 + cnt2,
                                             prevRowCnt5 + nextColCnt5 + cnt5));
                res = std::max(res, std::min(nextRowCnt2 + prevColCnt2 + cnt2,
                                             nextRowCnt5 + prevColCnt5 + cnt5));
                res = std::max(res, std::min(nextRowCnt2 + nextColCnt2 + cnt2,
                                             nextRowCnt5 + nextColCnt5 + cnt5));
            }
        }
