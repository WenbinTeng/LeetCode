...
public:
    int closestCost(std::vector<int>& baseCosts, std::vector<int>& toppingCosts, int target) {
        for (auto& base : baseCosts) if (res == target) return res; else dfs(toppingCosts, base, 0, target);
        return res;
    }
...
private:
    ...
    void dfs(std::vector<int>& toppingCosts, int sum, int index, int target)
    {
        ...
        if (std::abs(sum - target) == std::abs(res - target) && sum < res) res = sum;

        if (sum >= target || index == toppingCosts.size()) return;
