...
private:
    ...
    {
        if (i3 == s3.size()) return mem[i1][i2] = 1;

        if (mem[i1][i2] != -1) return mem[i1][i2];

        if (i1 != s1.size() && s1[i1] == s3[i3] && dfs(s1, s2, s3, i1 + 1, i2, i3 + 1)) return mem[i1][i2] = 1;
        if (i2 != s2.size() && s2[i2] == s3[i3] && dfs(s1, s2, s3, i1, i2 + 1, i3 + 1)) return mem[i1][i2] = 1;
