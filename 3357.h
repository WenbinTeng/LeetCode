...
private :
    bool check(std::vector<int>& nums, int k) {
        ...
        std::sort(interval.begin(), interval.end(), [](std::pair<int, int>& a, std::pair<int, int>& b){ return a.second < b.second; });
        ...
        for (int i = 0, j = -1; i < nums.size(); i++) {
            if (nums[i] >= 0) {
                auto validate = [&](){
                    if (j == -1 || j + 1 == i)
                        return true;
                    if ((std::abs(nums[j] - X) <= k && std::abs(nums[i] - X) <= k) || (std::abs(nums[j] - Y) <= k && std::abs(nums[i] - Y) <= k))
                        return true;
                    if (Y - X > k)
                        return false;
                    return true;
