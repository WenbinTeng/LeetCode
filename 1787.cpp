...
class Solution {
  ...
  private:
    int dfs(std::vector<std::vector<int>> &mem,
            std::vector<std::unordered_map<int, int>> &groups,
            std::vector<int> &numCnt, int index, int k, int last) {
        if (index == k) {
            return last == 0 ? 0 : INT_MAX;
        }
        ...
        for (const auto &[num, cnt] : groups[index]) {
            ...
            res =
                rec == INT_MAX ? res : std::min(res, numCnt[index] - cnt + rec);
        }
