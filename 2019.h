...
public:
    int scoreOfStudents(std::string s, std::vector<int>& answers) {
        ...
        seq = std::vector<std::vector<std::unordered_set<int>>>(_operands.size(), std::vector<std::unordered_set<int>>(_operands.size(), std::unordered_set<int>({})));
...
private:
    ...
    {
        if (!seq[l][r].empty()) return;

        if (l == r) { seq[l][r] = std::unordered_set<int>({_operands[l] - '0'}); return; }
        ...
        for (int i = l; i < r; ++i)
        {
            ...
            for (const auto& nl : seq[l][i + 0])
            {
                ...
                {
                    if (_operator[i] == '+' && nl + nr <= 1000) res.insert(nl + nr);
                    if (_operator[i] == '*' && nl * nr <= 1000) res.insert(nl * nr);
                }
            }
        }
