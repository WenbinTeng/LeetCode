...
class Solution {
  public:
    ListNode *mergeKLists(std::vector<ListNode *> &lists) {
        ...
        while (dataVec[treeVec[0]] != this->MAX_VAL) {
            ...
            dataVec[minIndex] =
                iterVec[minIndex] ? iterVec[minIndex]->val : this->MAX_VAL;
  ...
  private:
    ...
    void createLoserTree(std::vector<int> &treeVec, std::vector<int> &dataVec,
                         int k) {
        for (int i = k - 1; i >= 0; i--) {
    ...
    void adjustLoserTree(std::vector<int> &treeVec, std::vector<int> &dataVec,
                         int nodeNum, int k) {
